// ═══════════════════════════════════════════════
//  CAPTA — Core JS v2.1
//  Database: Google Sheets via Apps Script (GET)
// ═══════════════════════════════════════════════
...
// ── Google Sheets API via GET (no CORS issue) ─
const GS = {
  async call(params) {
  ...
  async syncAll() {
    ...
    for (const s of sheets) {
      const data = await this.getAll(s);
      if (data !== null) DB.set(s, data);
    }
    return true;
  }
};

// ── Smart DB (local + GS sync) ────────────────
const SDB = {
  async push(key, item) {
    const saved = DB.push(key, item);
    if (CONFIG.SCRIPT_URL) {
      const r = await GS.append(key, saved);
      if (!r?.ok) console.warn('GS append failed for', key);
    }
    return saved;
  },
...
function getHarga(kode, platform) {
  ...
  return ({ 'Shopee':p.harga_shopee, 'Tokopedia':p.harga_tokopedia, 'TikTok Shop':p.harga_tiktok })[platform] || p.harga_shopee || 0;
}
...
function pct(n) { return (!n&&n!==0)?'—':(+n*100).toFixed(1)+'%'; }
...
function buildSidebar(activePage) {
  const user = getUser();
  const gsStatus = CONFIG.SCRIPT_URL
    ...
    <div class="sidebar-foot">
      <div class="user-row">
        ...
        <div>
          ...
          <div class="user-role">${gsStatus}</div>
        </div>
      </div>
...
async function initPage(activePage, title, sub) {
  ...
  // Sync dari Google Sheets jika terhubung
  if (CONFIG.SCRIPT_URL) {
    toast('Menyinkronkan data...', 'info');
    const ok = await GS.syncAll();
    if (ok) toast('Data tersinkron ✓', 'success');
    else toast('Gagal sinkron, pakai data lokal', 'info');
  }
